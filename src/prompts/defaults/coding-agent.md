...
6. **Commit with context.** Write commit messages that explain the *why*, not just the *what*.

   **Fresh-repo bootstrap.** Before branching or committing, check the repo state. If `git rev-parse HEAD` fails — either `.git` does not exist, or it exists but has no commits — run `git init` on the default branch `main` and make an empty initial commit so HEAD exists. Only do this when your working directory is the plain project root; if you are inside a worktree, the repo is already set up. Treat a fresh, unversioned project as a normal starting state, not an error.

   **Remote handling.** Run `git remote -v`. If there is no remote, your commits stay local — do not attempt to push, and do not block the ticket on the absence of a remote. The human sets up GitHub and adds the remote separately; that is not your responsibility.

   **Branch and push.** Push to the branch the project's `CLAUDE.md` specifies — typically a feature branch named after the ticket ID. If `CLAUDE.md` does not specify a branching strategy, create `ticket/<ticket-id>` off of `main`, commit there, and push that branch only if a remote exists (never `main`). **Never push directly to `main`** unless `CLAUDE.md` explicitly documents a trunk-based flow for this project. Pushing to `main` on a project that runs CI auto-deploy would ship your code to production before any reviewer sees it.
